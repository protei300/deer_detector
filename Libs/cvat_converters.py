    ...
    def from_cvat(self, folder_to: Path, folder_from: Path, *args, **kwargs):
        ...
        if folder_from.is_file():
            with TemporaryDirectory() as tmp:
                ...
                df['RelPath'] = df['Label'].map(lambda x: y[-1] if (y:=x.parent.parts)[-1] != 'images' else y[-2])
...
class YOLOBoxOBBConverter(BaseConverter):

    def __init__(self):
        super().__init__('CVAT 1.1', 'YOLO_OBB')
    ...
    def from_cvat(self, folder_to: Path, folder_from: Path, *args, **kwargs):
        ...
        if folder_from.is_file():
            with TemporaryDirectory() as tmp:
                working_dir = Path(tmp)
                unpack_archive(folder_from, working_dir, format='zip')
                self._from_cvat(folder_to, working_dir, kwargs)
        else:
            self._from_cvat(folder_to, folder_from, kwargs)


    def _from_cvat(self, folder_to: Path, folder_from: Path, kwargs):


        is_obb = kwargs.get('is_obb', False)
        ...
        for child in tqdm(root.iter('image')):
            ...
            rel_pic_loc = Path(child.attrib['name'])
            if len(rel_pic_loc.parts) <= 2:
                other, doctype, filename = '', '', rel_pic_loc.parts[-1]
            else:
                other, doctype, filename = '\\'.join(rel_pic_loc.parts[:-2]), rel_pic_loc.parts[-2], rel_pic_loc.parts[-1]
            ...
            for box in child:
                ...
                # getting points
                x_u, y_u, x_b, y_b, rotation = (box.attrib['xtl'], box.attrib['ytl'], box.attrib['xbr'],
                                             box.attrib['ybr'], box.attrib.get('rotation', '0.0'))

                points = [x_u, y_u, x_b, y_b, rotation]
                points = list(map(lambda x: float(x), points))
                if is_obb:
                    points = self.xyxya2xyxyxyxy(*points).ravel().tolist()
                else:
                    points = points[:-1]


                points[::2] = list(map(lambda x: str(round(x/w, 5)), points[::2]))
                points[1::2] = list(map(lambda y: str(round(y/h, 5)), points[1::2]))
    ...
    @staticmethod
    def xyxya2xyxyxyxy(x_u, y_u, x_b, y_b, angle: float):
        """
        Converts 2 dots and angle rectangle to 4 dots rectangle
        :param x_u:
        :param y_u:
        :param x_b:
        :param y_b:
        :param angle:
        :return: np.array matrix with 4 dots xy xy xy xy
        """
        x_c, y_c = (x_u + x_b) // 2, (y_u + y_b) // 2
        rot_mtrx = cv2.getRotationMatrix2D((x_c, y_c), 360 - angle, 1.)
        x1, y1 = np.dot(rot_mtrx, np.array((x_u, y_u, 1)).T)
        x2, y2 = np.dot(rot_mtrx, np.array((x_b, y_u, 1)).T)
        x3, y3 = np.dot(rot_mtrx, np.array((x_b, y_b, 1)).T)
        x4, y4 = np.dot(rot_mtrx, np.array((x_u, y_b, 1)).T)
